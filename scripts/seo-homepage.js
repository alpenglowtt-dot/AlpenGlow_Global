...
h = h.replace(/(?:\r?\n)*<!-- SEOCONTENT:BEGIN -->[\s\S]*?<!-- SEOCONTENT:END -->(?:\r?\n)*/g, '\n');
...
.footer-all-link { display: inline-block; color: var(--orange-light); font-size: 0.85rem; font-weight: 700; letter-spacing: 0.01em; line-height: 1.5; margin-top: 1.4rem; padding-top: 1rem; border-top: 1px solid rgba(255,255,255,0.1); transition: color 0.3s; }
.footer-all-link::after { content: '\\2192'; display: inline-block; margin-left: 0.35rem; transition: transform 0.3s; }
.footer-all-link:hover { color: var(--orange); }
.footer-all-link:hover::after { transform: translateX(3px); }
 ...
 * The postal address lives as static markup in the footer's Contact Us column instead
 * of here, so it isn't duplicated on every run. */
